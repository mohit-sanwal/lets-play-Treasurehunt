		...
		return(
			<div>
			  ...
			  {this.state.columNum.length>0 && this.state.rowNum.length>0 ? "":  <div >
				...
				{this.state.columNum.length>0 && this.state.rowNum.length>0 ? "" : <button className="bt btn-success btn_click"  onClick = {this.onInitialSubmit}> click 3 Times </button>}
 				</div>}
				...
				{this.state.showFinalGrid ? <div>{newTable}</div> : "" }
				 ...
				 {this.state.columNum.length>0 && this.state.rowNum.length>0  ? <button type="button" className="bt btn-danger btn_click" onClick = {this.onFinalSubmit.bind(this)}>FinalSubmit</button> : " "}
		   </div>
		)
	}
